...
function check_task_foundation_v2_gate() {
  const checks = [
    ...
    {
      file: 'src/core/records/schema/definitions.ts',
      forbidden: [/-\s*\[\s*\]/, /-\s*\[x\]/i, /taskStatusPrefix/, /taskDateToken/, /repeatToken/],
      message: 'core.task registration must use canonical Record v2 fields.',
    },
    {
...
function check_task_domain_v2_gate() {
  ...
  requireText('src/core/types/cache.ts', 'CURRENT_CACHE_SCHEMA_VERSION = 15');
...
function check_no_task_line_runtime_gate() {
  ...
  if (!cache.includes('CURRENT_CACHE_SCHEMA_VERSION = 15')) failures.push('cache.ts: cache schema must be v14 after integrity persistence hardening');
