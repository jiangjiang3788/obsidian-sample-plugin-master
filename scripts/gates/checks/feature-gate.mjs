...
// ---------------------------------------------------------------------------
// scripts/gates/checks/feature-gate.mjs
// ---------------------------------------------------------------------------
...
const allowlistPath = path.join(ROOT, 'scripts', 'gates', 'feature-gate.allowlist.json');
/** @type {Set<string>} */
const allow = new Set();
...
if (staleAllow.length) {
  failWithViolations('feature-gate', staleAllow.map((k) => ({
    file: path.join(ROOT, 'scripts', 'gates', 'feature-gate.allowlist.json'),
    loc: '0:0',
