...
> [!success] **已完成 (2025-11-24)**
> - `src/main.ts` 已清理，移除了 Polyfill 代码。
> - 启动逻辑已模块化，Polyfill 移至 `src/core/polyfills.ts`。
...
> [!success] **已完成 (2025-11-24)**
> - `src/core/services/DataStore.ts`: 已添加标准 JSDoc，明确 Service 职责。
> - `src/app/AppStore.ts`: 已添加标准 JSDoc，明确 Store 职责。
...
> [!info] **进度 (2025-11-24)**
> - 已检查 `src/core/di/setupCore.ts`，DI 容器配置已独立。
