# ThinkOS 白板 — 实施基线 v1.2.5 R2

> 当前版本：**1.2.5 R2 Nested Reliability：二级/深层工作台退出可靠性 + 左侧 Record 拖入当前工作台可靠性 + 重启恢复合同。**
...
### 9.1 1.2.5 R2 — Nested Reliability 修正

针对真机反馈“二级工作台退不出来 / 左侧 Record 拖不进工作台”，R2 收口以下可靠性合同：

- **永远有显式退出路径**：进入任意 Nested Workbench 后，顶部固定显示“返回上一级工作台”和“退出到根白板”两个按钮；`Esc` 在没有输入/编辑/selection 优先消费时返回父级。
- **breadcrumb 不再被 Canvas 覆盖**：breadcrumb 改为 absolute overlay，并处在 Canvas viewport 之上；根 crumb 明确标注“退出到根白板”。
- **左侧 Source drop 以 pointer world point 命中 Workbench**：不再用“未来卡片中心”做 target hit-test，避免靠近 Workbench 边缘时指针明明在框内却误落根白板。
- **当前全屏 Workbench 是可靠 fallback target**：进入二级/深层工作台后，从左侧拖到当前 canvas 任意有效位置，若没有命中更深子 Workbench，则 `groupId = activeGroupId`。
- **深层子 Workbench 优先**：若当前工作台内还显示更深子 Workbench，pointer 落在其 frame 内时选最深可见 target。
- **拖拽有目标反馈**：Source drag 期间，目标 Workbench 使用 drop-target 高亮，顶部 drop hint 显示具体工作台名称。
- **重启合同不变**：active nested canvas / history / camera / zoom 是 ephemeral，因此重启回根白板；但卡片 `groupId/x/y`、Workbench 层级等 durable 数据必须完整恢复。
- R2 不增加 schema 字段、不升 `whiteboards.json` data version、不进入 1.2.6。
...
额外全项目 `src + test + scripts` TypeScript parser 扫描：1025 files / 0 syntax error。
...
R2 首要回归：
- 进入二级、三级、四级工作台后，顶部“返回上一级 / 退出到根白板”始终可见可点；Esc 可逐层退出；
- 根 breadcrumb / Parent / Back / Forward 不会被 Canvas 覆盖；
- 左侧单条 Record 拖到二级全屏工作台后，持久化 `item.groupId` 必须是当前二级工作台；
- 左侧多选后拖一条，整批都落入当前工作台；
- 在父工作台画布中把 Source Record 放到可见子 Workbench frame 的边缘附近，也必须命中该 Workbench；
- 重启后 active canvas 回根白板是预期；再次进入原父/子 Workbench，卡片仍在原 `groupId/x/y`，且不重新出现在左侧候选。
...
## 1.2.5 R3 — Nested 导航与极端 Zoom 找回可靠性修复

- 子画布退出入口以 `activeGroupId` 为唯一显示条件，不再依赖 breadcrumb/path/title 是否同步完成；只要当前处于 Workbench 子画布，就必须出现“返回上一级工作台 / 退出到根白板”。
- “回到画布中心”继续恢复 100%，但不再使用所有内容包围盒的空白几何中心；改为 content-aware home：使用坐标中位数抵抗远端离群节点，再锚定到一个真实 Card / Workbench / Annotation，因此找回后至少有真实内容进入视野。
- 进入 Workbench、返回父级、返回根白板同样使用 content-aware home，避免被极远成员撑大的 Workbench frame 中心带到空白区。
- 新增全层级“适配当前画布内容”入口；当内容跨度超过最小 zoom 仍无法一次 Fit 时，退化为最小 zoom + 真实内容 home，而不是停在空白几何中心。
- `ThinkIconButton` 支持调用方自定义 tooltip；“回到画布中心”会明确说明当前是根白板还是具体 Workbench，以及会恢复 100%。
- 不改变 durable x/y、groupId、Archive、Edge、Record 真源；camera/zoom/navigation 继续 ephemeral。
