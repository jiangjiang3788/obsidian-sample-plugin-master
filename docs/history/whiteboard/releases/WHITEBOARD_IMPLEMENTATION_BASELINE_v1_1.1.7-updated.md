...
> 当前版本：**1.1.7 Workbench 分组 / 命名 / 折叠 / 整组移动。**
...
```text
┌──────────────────────────────────────────────────────────────┐
│ ThinkOS · 白板          定位 / 缩放 / Pan / Workbench       │
├─────────────────────┬────────────────────────────────────────┤
...
- 多选；
- 整组移动；
- 不提前引入第三方 infinite-canvas engine。

### 1.1.9 — Archive / Restore 原位置

- 白板 projection 归档；
- 恢复时保留原 world 位置；
- 不删除 canonical Record。
...
- 白板仍是 ThinkOS 一级 Workspace；不回到 Dashboard View/Layout。
- 1.1.6 左侧多选/全选完整 matched results/批量拖、回到画布中心继续兼容。
- 右上角可新建 Workbench；新建位置跟随当前 camera/zoom 的 viewport world 中心，可为负坐标。
- 现有卡片拖入展开 Workbench 后持久化 `groupId`；拖入另一 Workbench 可切换；“移出工作台”只清 membership。
- 左侧单条/批量 Record drop 到 Workbench 时成员关系和卡片新增在同一 `addRecords()` mutation 中落盘。
- Workbench 可命名、折叠/展开；折叠隐藏成员卡和相关 Edge，但不重排、不改 durable XY、不让成员重新出现在左侧候选。
- Workbench 标题栏/拖动手柄可整组移动；成员卡与 Edge 同步预览；pointerup 单次持久化 group anchor + 全部成员 XY。
- 解散 Workbench 不删除任何白板卡片或 canonical Record；成员位置原样保留。
- Find 命中折叠成员时只移动 camera 到工作台 frame；不自动展开、不写盘。
- “回到画布中心”将卡片与 Workbench frame 一起计算；空 Workbench 也可被找回。
- `groups/groupId` 对旧 1.1.6 文件保持可选兼容；Store data version 不升级；schema 拒绝重复 group ID 与悬空 groupId。
- F134 已登记 unit/ui/integration/persistence/restart/e2e/regression 证据；1.1.8 Canvas 框选/右侧多选未提前实现。
- Syntax / Language / Evidence / Surface / Architecture / Records / Task Session / Energy / UI Runtime 可执行审计通过。
- Quality / Stability / Product 仅保留 1.1.6 基线已有 any budget / 缺 CI / README release-contract 红项，数值与原因未新增。
- Jest / 完整 Typecheck / Build / Obsidian E2E 仍需在安装项目依赖的用户环境补跑。
- 下一步：**Hold 1.1.8，先真机验收 1.1.7。**
