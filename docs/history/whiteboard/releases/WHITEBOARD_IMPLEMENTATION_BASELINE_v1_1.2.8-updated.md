# ThinkOS 白板 — 实施基线 v1.2.8

> 当前版本：**1.2.8 Semantic Overview batch（1.2.6–1.2.8）**。
...
- `Think/whiteboards.json` 继续使用 `version: 1`；1.2.8 不增加 durable 字段，不做数据迁移。
...
与 untouched 1.2.5 R3 基线相同、不是本批新增的红项：
...
### 1.2.9 — Performance（条件版本）

先用 1.2.8 的 semantic LOD 做真实大白板压力测试。只有仍有明确卡顿才实施：
