...
> 当前版本：**1.1.8 R2 Canvas 框选 / 多选 / Workbench 批量交互 / 四边拖线。**
...
## 10. 当前核心产品验收（1.1.8 R2）

- 白板仍是 ThinkOS 一级 Workspace；1.1.0–1.1.7 数据合同继续兼容。
- 普通空白拖动仍 Pan；`Ctrl/⌘ + 拖动空白` 才框选；Shift 单独拖空白不框选。
- 框选在负 camera / 任意 zoom 下按 world 坐标命中当前可见卡片。
- `Ctrl/⌘/Shift + 点击` 可逐条增减选择；选择数量、清空、Escape 保持可用。
- 多选拖动保持相对位置；未命中 Workbench 时保留原 groupId。
- 多选拖入展开 Workbench 时，整批 selection 通过一次 `moveItems(..., groupId)` 加入同一工作台。
- 多选拖到左栏或点击已选卡“移出”时，整批通过一次 `removeItems()` 移出，并清理 incident edges；canonical Record 保留。
- 四边 drag-to-connect 可直接创建 A→B edge；不再使用“连线 / 连到这里”按钮。
- connection preview / selection / marquee / camera / zoom 均不写盘；durable XY / groupId / edge 正常持久化。
- `whiteboards.json version: 1`，不引入复杂 Edge schema、Archive、culling 或 AI。
- Language / Evidence / Surface / Architecture / Records / Task Session / Energy / UI Runtime 可执行审计通过。
- Quality / Stability / Product 只保留原 1.1.8 基线红项，无新增。
- Jest / 完整 Typecheck / Build / Obsidian E2E 仍需在安装依赖环境补跑。
- 下一步：**Hold 1.1.9，先真机验收 1.1.8 R2。**
