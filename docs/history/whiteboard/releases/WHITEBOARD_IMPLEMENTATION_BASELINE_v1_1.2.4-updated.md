# ThinkOS 白板 — 实施基线 v1.2.4

> 当前版本：**1.2.4 Nested Workbench + Undo/Redo + Annotation + Edge Label + Arrange/Context Menu。**
...
- `Think/whiteboards.json` 继续使用 `version: 1`；1.2.4 只增加 optional 字段，1.1.9 数据可直接读取，不做强制迁移。
...
- 不在 1.2.4 做 parent-local durable coordinate 迁移；
- 不做 rich-text annotation editor。

## 10. 下一阶段路线

### 1.2.5 — Nested Navigation Polish（下一版）

- nested canvas Back / Forward history；
- 当前 Workbench “适配内容 / Fit content”；
- Nested Find 路径展示与更明确的 reveal；
- Workbench 全屏/父级/当前路径导航细节收口；
- annotation / empty child Workbench 的内容包围盒适配。
...
与 untouched 1.1.9 基线完全相同、非本版新增的红项：
...
- `⛶ 全屏`进入 Workbench 后 viewport 行为与根白板一致，breadcrumb 可返回；
