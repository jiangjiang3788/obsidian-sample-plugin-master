...
> 当前版本：**1.1.5 假无限白板 / Pan / World-Camera-Screen 坐标。**
...
### 1.1.5 — 假无限白板 / Pan / World-Camera-Screen（✅ 代码完成 / 待用户真机验收）
...
唯一目标：降低从候选区一次挑多条 Record 进入白板的操作成本。

- 左侧候选多选；
- 当前查询结果全选；
- 批量拖入白板；
- 不改变 canonical Record 真源与 RecordQuery 查询语义。
...
7. 用户可向四个方向 Pan，负 world 坐标卡片可拖入、拖动、连线并在重启后恢复；
8. Pan/Zoom/定位只改 ephemeral camera/zoom，不因连续视图操作写 `whiteboards.json`。
