...
### 1.1.3 — 白板内定位 + 工作区收纳（✅ 代码完成/待用户真机验收）
...
- 切换；
- 重命名；
- 删除/归档另行做数据安全设计，不自动 hard-delete。
...
- `Ctrl/⌘ + wheel` 与 `- / 100% / +`；
- 缩放围绕当前视口/指针，避免跳屏；
- WhiteboardItem 的 x/y 继续是未缩放的逻辑坐标，缩放不改 durable item/edge 数据；
- 左拖右、卡片拖动、Edge geometry 必须做 screen↔board 坐标换算；
- zoom 第一版保持 ephemeral，不持久化 camera；
- 不顺手引入 minimap / infinite-canvas engine / spatial index。
...
- 无限画布 engine；
- pan/minimap / 无限相机（缩放已排入 1.1.5，其他相机能力继续延期）；
