 ...
 * @covers F055/regression
 * @covers F056/integration
 * @covers F056/persistence
 */
...
describe('integration: TaskSession integrity', () => {
  ...
  it('reports the Session when an Energy reference resolves to a non-Energy Record', () => {
    const task = parse(encodeRecordBlock({
      recordId: 'task.01KZQ1WW00HN6JRP7K368XHHVB', recordType: 'task', fields: { status: 'done', content: '任务' },
    }), 'task.md');
    const thought = parse(encodeRecordBlock({
      recordId: 'rec.01KZQ1WW00HN6JRP7K368XHHVC', recordType: 'thought', fields: { 内容: '不是精力记录' },
    }), 'thought.md');
    const session = parse(encodeRecordBlock({
      recordId: 'tasksession.01KZQ1WW00HN6JRP7K368XHHVD', recordType: 'task-session',
      fields: {
        taskId: task.id,
        sessionStartedAt: '2026-08-11T15:24:00.000Z',
        sessionEndedAt: '2026-08-11T15:50:00.000Z',
