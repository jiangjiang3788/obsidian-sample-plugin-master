...
describe('RecordEntity R2 boundary', () => {
  it('keeps the canonical entity separate from the consumer projection', () => {
    const thought = record({ id: 'rec.thought', recordType: 'thought', content: '一个想法' });
    const view = toRecordViewItem(thought);
    expect(view).toBe(thought);
    expect(view.recordType).toBe('thought');
  });

  it('narrows Task, Series, Session and Habit only through typed projections', () => {
    const task = record({ id: 'task.a', recordType: 'task', status: 'open', seriesId: 'taskseries.a' });
    const series = record({ id: 'taskseries.a', recordType: 'task-series', status: 'active', recurrenceInfo: { unit: 'week', interval: 1, anchor: 'scheduled' } });
    const session = record({
      id: 'tasksession.a', recordType: 'task-session', taskId: 'task.a', seriesId: 'taskseries.a',
    ...
    const habit = record({ id: 'rec.habit', recordType: 'habit', rating: 4, image: 'x.png' });
  ...
  it('stores canonical entities internally while queries return explicit view projections', () => {
    ...
    const thought = record({ id: 'rec.thought', recordType: 'thought' });
    const session = record({
      id: 'tasksession.a', recordType: 'task-session', taskId: 'task.a',
      sessionStartedAt: '2026-08-11T09:00:00.000Z', sessionEndedAt: '2026-08-11T09:30:00.000Z',
      sessionDurationMinutes: 30, sessionResult: 'work-block-ended', sessionSource: 'timer',
    });
