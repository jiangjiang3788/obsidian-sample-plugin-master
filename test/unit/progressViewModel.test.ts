...
import {
  ...
  getVisibleProgressGoalBreakdown,
  progressBarWidth,
  ratioPercent,
...
describe('ProgressView goal mode', () => {
  ...
  it('honors topN without introducing internal filters', () => {
    const model = buildProgressViewRenderModel({ items: items as never, module: { viewConfig: { topN: 1 } }, goals: [] });
    expect(model.goalCards).toHaveLength(1);
  });
  ...
  it('groups recent records by child Goal inside each root Goal card', () => {
    ...
    expect((card as unknown as { recentRecords?: unknown }).recentRecords).toBeUndefined();
...
describe('ProgressViewModel', () => {
  ...
  it('derives labels, remaining points, collapsed facts and visible breakdowns', () => {
    expect(getGoalProgressTitle(card as unknown as GoalProgressCardModel)).toBe('工作/代码');
    expect(getProgressLeafLabel('工作/代码')).toBe('代码');
    expect(getGoalProgressRemainingPoints(card as unknown as GoalProgressCardModel)).toBe(70);
    expect(buildProgressCollapsedFacts(card as unknown as GoalProgressCardModel).map((fact) => `${fact.label}:${fact.value}`)).toEqual([
      '任务:2',
      '打卡:0',
      '阻碍项:0',
      '里程碑:1',
    ...
    expect(getVisibleProgressGoalBreakdown(card.goalBreakdown).map((row) => row.key)).toEqual(['工作/代码']);
  });
