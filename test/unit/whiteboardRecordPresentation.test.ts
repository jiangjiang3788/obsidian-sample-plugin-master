...
function record(recordType: string, overrides: Partial<RecordViewItem> = {}): RecordViewItem {
  return {
    id: `rec.${recordType}`, recordType, title: `${recordType} title`, content: `${recordType} content`, tags: [],
    goalPath: '健康/运动', date: '2026-09-01', created: 0, modified: 0, extra: {}, ...overrides,
  };
}
...
describe('白板 Record presentation', () => {
  it('保留 mixed Record Type identity、目标/时间与类型特有摘要', () => {
    ...
    const feeling = buildWhiteboardRecordPresentation(record('feeling', { title: '', content: '今天很平静' }));
    expect(feeling.typeLabel).toBe('感受');
    expect(feeling.primaryText).toBe('今天很平静');
