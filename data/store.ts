// data/store.ts - 数据存储与查询
// 仅展示 **完整文件**，在原基础上新增 notifyChangeImmediate()
...
export class DataStore {
      ...
      results.sort((a, b) => {
        for (const rule of sortRules) {
          ...
          if (typeof aVal === 'number' && typeof bVal === 'number') {
            if (aVal !== bVal)
              return rule.dir === 'asc' ? aVal - bVal : bVal - aVal;
          } else {
  ...
  private _matchItem(item: Item, rule: FilterRule): boolean {
    ...
    if (rule.op === '=' || rule.op === '!=') {
      const isEqual =
        fieldVal != null && cmpVal != null && String(fieldVal) === String(cmpVal);
      return rule.op === '=' ? isEqual : !isEqual;
    }
    if (rule.op === 'includes') {
      if (fieldVal == null) return false;
      if (Array.isArray(fieldVal))
        return fieldVal.some(v => String(v).includes(String(cmpVal)));
