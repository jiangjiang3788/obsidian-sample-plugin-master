...
    // [REFACTOR] 使用常量作为 ID
    this.app.setting.setTabById?.('think-settings'); // SettingsTab 的 ID 是硬编码的，暂不改动
    if (name) {
      // [REFACTOR] 使用常量作为 localStorage 的键
      localStorage.setItem(LOCAL_STORAGE_KEYS.TARGET_DASHBOARD, name);
    }
  }
}