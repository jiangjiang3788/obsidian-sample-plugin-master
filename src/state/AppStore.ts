...
import { useState, useEffect, useCallback } from 'preact/hooks'; // [修改] 导入 useCallback
...
        if (DEBUG_MODE) {
            this.notifyCounter++;
            console.log(`[AppStore] 状态通知 #${this.notifyCounter}`, { listeners: this._listeners.size });
        }
...
// [核心修复] 修正 useStore Hook 的实现
export function useStore<T>(selector: (state: AppState) => T): T {
    const store = AppStore.instance;

    // 使用 useCallback 确保 getSnapshot 函数引用稳定
...
            // 使用函数式更新，避免依赖旧的 state 值
            setState(currentSnapshot => {
                if (newSnapshot !== currentSnapshot) {
                    if (DEBUG_MODE) console.log('[useStore] 状态切片已更新，触发组件重渲染。');
                    return newSnapshot;
                }
...
        // 同时，为了防止在首次渲染和useEffect执行之间发生状态变更，
        // 我们再检查一次。
...
        // 组件卸载时，返回取消订阅的函数
        return unsubscribe;
        // [核心修复] 移除依赖数组中的 `state`，确保订阅只在 selector 变化时重置
