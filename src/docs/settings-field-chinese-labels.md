...
- `src/features/settings/layout/ModuleSettingsModal.tsx`
  - 显示字段、分组字段传入 `getFieldLabel`。

- `src/features/settings/viewEditors/RuleBuilder.tsx`
  - 筛选/排序字段下拉显示中文。
  - 已添加规则 Chip 显示中文字段名。

- `src/features/settings/viewEditors/TableViewEditor.tsx`
  - 交叉表行字段、列字段显示中文。
