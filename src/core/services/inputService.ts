...
import { AppStore } from '@state/AppStore'; // [MOD] 导入 AppStore
import type { ThinkSettings } from '../../main'; // [MOD] 导入顶层设置类型以获得类型提示
...
  // [MOD] 移除 plugin 依赖，现在服务更独立
  constructor(private app: App) {}

  /* ------------------------------------------------------------------ */
  /*  读取插件设置                                                      */
  /* ------------------------------------------------------------------ */
  private get settings(): ThinkSettings['inputSettings'] {
    // [MOD] 直接从 AppStore 单例获取设置，这是唯一真源
    return AppStore.instance.getSettings().inputSettings || { base: {}, themes: [] };
  }
