...
function isOptionObject(
  value: unknown,
): value is { label?: unknown; value?: unknown } {
  return (
    !!value &&
    typeof value === "object" &&
...
function isPathLikeField(field: TemplateField): boolean {
  ...
  if (
    ...
    field.options.some((opt) => String(opt?.value || "").includes("/"))
  )
    return true;
  return false;
}

function isCategoryLikeField(field: TemplateField): boolean {
  if (!field) return false;
  const key = String(field.key || field.label || "");
  return key.includes("分类") || /category/i.test(key);
}

function isPeriodField(field: TemplateField): boolean {
...
function isTagsField(field: TemplateField): boolean {
  const key = normalizeToken(field?.key);
  const label = normalizeToken(field?.label);
  return (
    ["标签", "tag", "tags"].includes(key) ||
    ["标签", "tag", "tags"].includes(label)
  );
}

function readPeriodFromLegacyCategory(
  field: TemplateField,
  item: Item,
...
function mapFieldValue(field: TemplateField, rawValue: unknown): unknown {
  ...
  if (["select", "radio", "rating"].includes(field.type)) {
    ...
    if (field.type === "rating" || field.semanticType === "ratingPair") {
      const option = options.find(
        (opt) =>
          String(opt.label ?? opt.value) === String(rawValue) ||
    ...
    if (isPathLikeField(field)) {
      ...
      const option = options.find(
        (opt) =>
          normalizePath(String(opt.value || "")) === rawPath ||
          String(opt.label || "") === String(rawValue),
      );
      if (option)
        return {
          value: normalizePath(String(option.value)),
          label:
            option.label || getLeafPath(String(option.value)) || option.value,
        };
...
function buildRatingOption(field: TemplateField, item: Item) {
  ...
  let option = options.find(
    (opt) =>
      String(opt.label ?? "") === score &&
      (!image || String(opt.value || "") === image),
  );
  if (!option && score)
    option = options.find((opt) => String(opt.label ?? opt.value) === score);
  if (!option && image)
    option = options.find((opt) => String(opt.value || "") === image);
  if (option)
    return { value: option.value, label: option.label || option.value };
...
function isContentField(field: TemplateField): boolean {
  const key = normalizeToken(field?.key);
  const label = normalizeToken(field?.label);
...
function isTitleField(field: TemplateField): boolean {
  const key = normalizeToken(field?.key);
  const label = normalizeToken(field?.label);
  return ["title", "标题"].includes(key) || ["title", "标题"].includes(label);
}

function readSnapshotSemanticValue(
  field: TemplateField,
  item: Item,
  snapshot: ParsedRecordSnapshot,
): unknown {
  ...
  if (isTitleField(field))
    return (
      snapshot.semantic.title || snapshot.semantic.editableText || item.title
    );

  if (["日期", "date"].includes(key) || ["日期", "date"].includes(label))
    return snapshot.semantic.date;
  if (isPeriodField(field))
    return (
      snapshot.semantic.period ||
      readPeriodFromLegacyCategory(field, item, snapshot)
    );
  if (isTagsField(field))
    return (
      formatTagsForField(snapshot.semantic.tags) ||
      formatTagsForField(item.tags)
    );
  if (
    ["时间", "time", "start", "starttime"].includes(key) ||
    ["时间", "time", "start", "starttime"].includes(label)
  )
    return snapshot.semantic.startTime;
  if (
    ["结束", "end", "endtime"].includes(key) ||
    ["结束", "end", "endtime"].includes(label)
  )
    return snapshot.semantic.endTime;
  if (
    ["时长", "duration"].includes(key) ||
    ["时长", "duration"].includes(label)
  )
    return snapshot.semantic.duration;

  if (
    ["主题", "theme", "themepath", "完整主题", "完整路径主题"].includes(key) ||
    ["主题", "theme", "themepath", "完整主题", "完整路径主题"].includes(label)
  ) {
    return snapshot.semantic.themePath || item.theme || null;
  }
  if (
    ["roottheme", "根主题"].includes(key) ||
    ["roottheme", "根主题"].includes(label)
  )
    return snapshot.semantic.rootTheme;
  if (
    ["leaftheme", "叶主题"].includes(key) ||
    ["leaftheme", "叶主题"].includes(label)
  )
    return snapshot.semantic.leafTheme;
  if (isCategoryLikeField(field))
    return snapshot.semantic.categoryKey || item.categoryKey;
