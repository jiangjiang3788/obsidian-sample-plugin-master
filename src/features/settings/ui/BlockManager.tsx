...
// [CORE FIX] Added TextField to the import list from @mui/material
...
// 可排序的列表项 HOC (高阶组件)
function SortableBlockItem({ block, openId, setOpenId, handleDelete }) {
    const {
        attributes,
        listeners,
        setNodeRef,
...
    };
    ...
    // 拖动结束后的处理函数
    const handleDragEnd = (event: any) => { // Use 'any' to avoid type conflicts with dnd-kit
        ...
        if (active && over && active.id !== over.id) {
            ...
            // 直接更新整个数组到 AppStore
            const currentSettings = AppStore.instance.getSettings();
            const newInputSettings = { ...currentSettings.inputSettings, blocks: newSortedBlocks };
            AppStore.instance['_updateSettingsAndPersist'](draft => {
