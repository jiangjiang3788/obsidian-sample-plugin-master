...
export function FieldRow({ field, disabled = false, defaultValueLocked = false, isDragging = false, onUpdate, onRemove }: { field: TemplateField; disabled?: boolean; defaultValueLocked?: boolean; isDragging?: boolean; onUpdate: (updates: Partial<TemplateField>) => void; onRemove: () => void; }) {
  ...
  return (
    <div className={`think-field-row${isDragging ? ' is-dragging' : ''}`}>
      <div className="think-fields-editor__grid">
        ...
        <div className="think-field-row__cell">{showInlineDefaultValue ? <NativeTextInput label="" value={localDefaultValue} type={defaultInputType(uiType)} onInput={(value) => { setLocalDefaultValue(value); onUpdate({ defaultValue: value }); }} onBlur={() => onUpdate({ defaultValue: localDefaultValue })} disabled={disabled || defaultValueLocked} placeholder={defaultValueLocked ? '继承目标图标' : '可留空'} className="think-settings-full-width" /> : <span className="think-field-row__empty" />}</div>
      ...
      {detailsOpen && (
        <div className="think-field-row__details">
          {uiType === 'textarea' && showDefaultValueEditor && <NativeTextarea label="默认值" value={localDefaultValue} rows={3} onInput={(value) => { setLocalDefaultValue(value); onUpdate({ defaultValue: value }); }} onBlur={() => onUpdate({ defaultValue: localDefaultValue })} disabled={disabled || defaultValueLocked} placeholder={defaultValueLocked ? '继承目标图标' : '可留空'} className="think-settings-full-width" />}
