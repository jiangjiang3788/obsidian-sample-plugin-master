...
function BlockEditor({ block, useCases }: { block: RecordCaptureTemplate; useCases: UseCases }) {
  ...
  return (
    <div className="think-block-editor think-settings-stack think-settings-stack--tight">
      ...
      <div className="think-settings-row"><span className="think-settings-row__label">目标文件</span><ThinkInput value={localBlock.targetFile} onInput={(e) => setLocalBlock((current) => ({ ...current, targetFile: (e.currentTarget as HTMLInputElement).value }))} onBlur={() => handleBlur('targetFile')} placeholder="{{goalPath}}/{{标题.value}}.md" /></div>
      <div className="think-settings-row"><span className="think-settings-row__label">追加标题</span><ThinkInput value={localBlock.appendUnderHeader || ''} onInput={(e) => setLocalBlock((current) => ({ ...current, appendUnderHeader: (e.currentTarget as HTMLInputElement).value }))} onBlur={() => handleBlur('appendUnderHeader')} placeholder="## {{goalPath}}" /></div>
