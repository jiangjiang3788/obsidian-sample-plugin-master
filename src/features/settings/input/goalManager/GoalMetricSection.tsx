...
export function GoalMetricSection() {
  ...
  const [message, setMessage] = useState(''); const [metricGoalPath, setMetricGoalPath] = useState(activeGoalOptions[0]?.value || '');
  const selectedMetricGoal = goals.find((goal) => (goal.path) === metricGoalPath) || null; const selectedMetrics = selectedMetricGoal?.metrics || [];
  ...
  const syncMetricDraft = (goalPath: string) => { const goal = goals.find((item) => item.path === goalPath) || null; const first = goal?.metrics?.[0]; setMetricGoalPath(goalPath); if (first) { setMetricKey(first.key); setMetricLabel(first.label); setMetricDirection(first.direction); setMetricTargetValue(first.targetValue === undefined ? '' : String(first.targetValue)); setMetricUnit(first.unit || ''); } };
  ...
  const handleSaveMetric = async () => { if (!metricGoalPath) return; const key = metricKey.trim() || metricPresetKey(metricLabel); const label = metricLabel.trim() || key; const metric: GoalMetricContract = { key, label, direction: metricDirection, targetValue: metricTargetValue.trim() === '' ? undefined : Number(metricTargetValue), unit: metricUnit.trim() || undefined }; await useCases.goal.updateGoalMetrics(metricGoalPath, [...selectedMetrics.filter((item) => item.key !== key), metric]); setMessage(`已保存：${label}`); };
  const handleRemoveMetric = async (key: string) => { if (!metricGoalPath) return; await useCases.goal.updateGoalMetrics(metricGoalPath, selectedMetrics.filter((metric) => metric.key !== key)); setMessage('已删除指标'); };
  return (
    <section className="think-settings-section think-goal-metrics">
      ...
      <div className="think-settings-stack think-settings-stack--tight">
        <div className="think-settings-row"><span className="think-settings-row__label">目标</span><SimpleSelect value={metricGoalPath} options={activeGoalOptions} onChange={syncMetricDraft} placeholder="选择目标" fullWidth /></div>
        ...
        <div className="think-settings-row"><span className="think-settings-row__label">目标值</span><ThinkInput aria-label="目标值" className="think-settings-field--sm" type="number" value={metricTargetValue} onInput={(event) => setMetricTargetValue((event.currentTarget as HTMLInputElement).value)} /></div>
      ...
      <div className="think-settings-actions think-settings-actions--start"><ThinkButton variant="primary" size="sm" onClick={handleSaveMetric} disabled={!metricGoalPath || !metricLabel.trim()}>保存指标</ThinkButton></div>
