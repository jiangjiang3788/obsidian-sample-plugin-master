...
export function AiApiConfigSection({
  ...
  return (
    <ThinkDisclosure title="接口配置" open>
      <div className="think-settings-stack think-settings-stack--tight">
        <div className="think-settings-row"><span className="think-settings-row__label">接口地址</span><ThinkInput value={settings.apiEndpoint} placeholder="https://api.openai.com/v1" onInput={(e) => onUpdate({ apiEndpoint: (e.currentTarget as HTMLInputElement).value })} /></div>
        <div className="think-settings-row"><span className="think-settings-row__label">接口密钥</span><ThinkInput type="password" value={settings.apiKey} onInput={(e) => onUpdate({ apiKey: (e.currentTarget as HTMLInputElement).value })} /></div>
        ...
        <div className="think-settings-row"><span className="think-settings-row__label">最大生成长度（令牌数）</span><ThinkInput className="think-settings-field--md" type="number" value={settings.maxTokens} onInput={(e) => onUpdate({ maxTokens: parseInt((e.currentTarget as HTMLInputElement).value, 10) || 4096 })} /></div>
