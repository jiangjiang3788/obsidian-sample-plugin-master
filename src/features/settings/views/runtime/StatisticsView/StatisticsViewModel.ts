...
export function buildStatisticsViewConfig(module: ViewInstance, statisticsModel?: Partial<StatisticsRuntimeModel>): StatisticsViewConfig {
  return statisticsModel?.viewConfig ?? ({ ...STATISTICS_VIEW_DEFAULT_CONFIG, ...module.viewConfig } as StatisticsViewConfig);
}

export function resolveStatisticsStartDate(dateRange: [Date, Date], statisticsModel?: Partial<StatisticsRuntimeModel>): StatisticsDateLike {
  return statisticsModel?.startDate ?? dayjs(dateRange[0]);
}

export function isStatisticsYearView(currentView: StatisticsCurrentView, statisticsModel?: Partial<StatisticsRuntimeModel>): boolean {
  return statisticsModel?.isYearView ?? currentView === '年';
}

export function resolveStatisticsYear(startDate: StatisticsDateLike, statisticsModel?: Partial<StatisticsRuntimeModel>): number {
  return statisticsModel?.year ?? startDate.year();
}

export function resolveStatisticsBucketAccessor(statisticsModel?: Partial<StatisticsRuntimeModel>): (item: Item) => string {
...
export function resolveYearlyWeekStructure(input: {
  year: number;
  isYearView: boolean;
  statisticsModel?: Partial<StatisticsRuntimeModel>;
