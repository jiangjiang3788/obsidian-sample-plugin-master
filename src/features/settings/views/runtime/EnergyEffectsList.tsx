...
import type { GoalEnergyEffectRowModel, GoalEnergyEffectsModel } from '../models/energySummaryModel';

interface EnergyEffectsListProps {
...
export function EnergyEffectsList({ effects }: EnergyEffectsListProps) {
  ...
  return (
    <section class="think-progress-energy-effects" aria-label="活动前后精力变化分析">
      <div class="think-progress-energy-effects__header">
        <div>
          <strong>活动影响证据</strong>
          <span>仅在更多分析中保留详细证据</span>
        </div>
        <div class="think-progress-energy-effects__coverage">可配对 {effects.pairedActivityCount}/{effects.eligibleActivityCount}</div>
      </div>
      {noPairs ? (
        <div class="think-progress-energy-effects__empty">暂时没有可用的活动前后配对样本。</div>
      ) : (
        <>
          {(recovery || depletion) && (
            <div class="think-progress-energy-effects__discoveries">
              {recovery && <span>恢复：<strong>{recovery.label}</strong> {signed(recovery.meanDelta)}</span>}
              {depletion && <span>消耗：<strong>{depletion.label}</strong> {signed(depletion.meanDelta)}</span>}
            </div>
          )}
