...
import type { RecordViewItem, ViewInstance, InputSettings, ThemeDefinition, TemplateFieldOption } from '@core/types/public';
...
type HeatmapItemLike = RecordViewItem & {
    templateVariantId?: unknown;
    goalTemplateVariantId?: unknown;
    themePathNormalized?: unknown;
};

type HeatmapGoalTemplateLike = GoalTemplateStorageRow & {
    blockId?: unknown;
};
...
function readExtraText(item: RecordViewItem, key: string): string {
    return firstText(item.extra?.[key]);
}

function itemTemplateId(item: RecordViewItem): string {
    return firstText(item.templateId)
        || readExtraText(item, '模板ID')
        || readExtraText(item, 'templateId');
}

function itemTemplateVariantId(item: RecordViewItem): string {
    const heatmapItem = item as HeatmapItemLike;
    return firstText(heatmapItem.templateVariantId)
        || firstText(heatmapItem.goalTemplateVariantId)
...
function itemCoreBlock(item: RecordViewItem): string {
    const raw = firstText(item.coreBlock)
        || readExtraText(item, '核心Block')
        || firstText(item.categoryKey)
        || '';
...
function extractRatingOptions(template: GoalTemplateStorageRow | null | undefined): HeatmapRatingOptionLike[] {
    ...
    return Array.isArray(ratingField?.options)
        ? ratingField.options.map((option) => ({ value: option?.value, label: option?.label })).filter((option: HeatmapRatingOptionLike) => option.value !== undefined || option.label !== undefined)
        : [];
}
...
export function buildHeatmapViewModel(params: {
    ...
    const themesToTrack = Array.isArray(config.themePaths) && config.themePaths.length > 0
        ? config.themePaths.map((value) => String(value)).filter(Boolean)
        : inferredThemePaths;
    ...
    function resolvePresetMeta(item: RecordViewItem): PresetMeta | null {
